...
window.configure(background = "orange"); # Background color of window
window.option_add("*Font", "Times 16") # Setting the font-family & font-size 

usr_name = Label(window ,text = f"Connected to DB as: c##2020BTECS00005", background="orange").grid(row = 0, column = 1, pady=20)
...
tb_select = Label(window ,text = "Select the table: ", background="orange").grid(row = 1, column = 0, columnspan=1, padx=10, pady=10)
tb_dropdown = OptionMenu(window, variable, *DB_NAMES).grid(row = 1, column = 0, columnspan=2, padx=15)

def confirm_tb():
    global selected_tb
    tkinter.messagebox.showinfo("SUCCESS", f"Table {variable.get()} is selected!")
tb_btn = Button(window, text="Confirm", command=confirm_tb, background="green", foreground="white", border=5).grid(row = 1, column = 1)
...
def view_tb():
        ...
        newWindow.configure(background = "orange"); # Background color of window
        newWindow.option_add("*Font", "Times 16") # Setting the font-family & font-size
        global selected_tb
        
        Label(newWindow ,text = f"Viewing Table - {selected_tb}", background="orange").grid(row = 0, column = 0, padx=10, pady=10)
        ...
        c.execute(f'''select a.column_name
                       ...
                       where c.table_name='{selected_tb}' and c.constraint_type='P'
                        ''')
        for a in c:
            pk = a[0]
...
def insert_tb():
    ...
    Label(newWindow ,text = f"Insert values in table: {selected_tb}", background="orange").grid(row = 0, column = 0, padx=10, pady=10)
    c.execute(f'''SELECT column_name
                FROM USER_TAB_COLUMNS
                WHERE table_name = '{selected_tb}'
                ''')
    ...
    for ind, nm in enumerate(columns):
        Label(newWindow ,text = nm, background="orange").grid(row = ind+1, column = 0, padx=10, pady=10)
        ent = Entry(newWindow)
    ...
    def insert_val():
        ...
        if not is_empty:
            ...
            # Inserting values
            s = f'insert into {selected_tb}('+','.join(['?']*len(v))+')'+ ' values('+','.join([':?']*len(v))+')'
            for a in columns:
                s = s.replace('?', a, 1)
            for a in columns:
                s = s.replace('?', a, 1)
    ...
    try:
        ...
        if id is not None:
            ...
            if len(c.fetchall()) == 0:
                ...
                Label(newWindow ,text = "Update values in table:", background="orange").grid(row = 0, column = 0, padx=10, pady=10)
                c.execute(f'''SELECT column_name
                            FROM USER_TAB_COLUMNS
                            WHERE table_name = '{selected_tb}'
                            ''')
                ...
                def update_val():
                    ...
                    if not is_empty:
                        ...
                        s = f'update {selected_tb} set '+','.join(['? = :?']*len(v))+f' where {pk}={id}'
...
if selected_tb is not None:
    Label(window ,text = "Operations on selected table:", background="orange", font='Helvetica 18 bold').grid(row = 3, column = 0, padx=10, pady=60)
